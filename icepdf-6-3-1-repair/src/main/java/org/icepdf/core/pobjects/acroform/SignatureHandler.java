...
/**
 * The signature handler is responsible for returning validation results for a
 * given Digital signature's signature field dictionary. The returned Validation
 * objected can be interrogated to see which properties are considered valid.
 */
public class SignatureHandler {
	...
	static {
		...
		try {
			...
			// mgg-dev-it Security.insertProviderAt((Provider) provider, 2);
			Security.addProvider((Provider) provider); // mgg-dev-it
	 ...
	 * @return SignatureValidator object if cert and public key verified, null
	 *         otherwise.
	 */
	public SignatureValidator validateSignature(SignatureFieldDictionary signatureFieldDictionary) {
		...
		if (signatureDictionary != null) {
			...
			try {
				SignatureValidator signatureValidator = DigitalSignatureFactory.getInstance()
						.getValidatorInstance(signatureFieldDictionary);
